...
也可以点击左侧 Activity Bar 的“运行和调试”（`Ctrl+Shift+D`），展开“运行配置”区域。这里会直接列出当前工作区的配置：单击配置名称打开对应表单，使用配置右侧的运行/调试按钮可立即启动，标题栏的齿轮和刷新按钮用于编辑全部配置或重新读取 `launch.json`。Go 编辑器右上角的运行菜单中也提供“编辑运行/调试配置”入口。
