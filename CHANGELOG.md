...
- 更新运行/调试配置演示图，并补充“日志输出位置”各选项的作用和适用场景。
- 延续集成终端日志查找优化：保留上下文、增强命中高亮，并使用 `Enter` 向下和 `Shift+Enter` 向上查找。

## 0.5.5

- 修正集成终端查找的方向键位：`Enter` 查找下一项（向下），`Shift+Enter` 查找上一项（向上），与编辑器查找习惯保持一致。
- 增强深色与浅色主题中的终端查找高亮；当前命中使用更醒目的背景和边框，其他命中使用较弱的同色系高亮，并在滚动条概览中标记匹配位置。

## 0.5.4

- 运行配置表单将“控制台”明确为“日志输出位置”，说明集成终端的查找与调试控制台的筛选差异。
- 新建的本地 Go 启动配置和项目模板默认使用 `integratedTerminal`，让 `Ctrl+F` 定位日志时保留上下文；已有配置保持不变，可在表单中切换。
- 将集成终端回滚缓冲区提高到 `100000` 行，便于查找长时间运行服务的历史日志。

## 0.5.3

- 关闭项目、父目录和全局 Git ignore 对 `Ctrl+P` 与文件搜索的过滤，使当前打开工程中的未跟踪或被 Git 忽略文件也能通过文件名找到。
- 继续尊重用户显式配置的 `files.exclude` 和 `search.exclude`，避免将版本库内部目录或依赖缓存无条件加入快速打开结果。

## 0.5.2

- 修复扩展首次激活时“查找用法”分类节点已显示结果数量，但默认展开后偶尔没有渲染明细的问题；现在会先创建并聚焦底部视图，再发布完整结果树。
...
- 引用结果由 `gopls` 提供，并按声明、写入值、读取值分类，再按工作区、目录、文件、接收器结构体和函数/方法分组。
- 引用叶子节点显示代码摘要，单击即可打开文件并精确选中对应标识符；工具窗口支持刷新、清空和全部折叠。
- 匿名书签改用无冲突的 `Shift+Alt+B`，彻底释放 `F11` 给 VS Code 调试“单步进入”。
