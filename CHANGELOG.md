...
- 将编辑器右键菜单中的复制引用、书签和常用 Go 操作统一收进靠上的“GoLand Style”总菜单，减少顶层菜单长度。
- 关闭官方 Go 扩展原先散落在底部的可配置菜单项，并通过“Go 工具”子菜单保留添加 import、结构体标签、切换测试文件和光标处运行/调试测试。
- 保留 VS Code 原生剪切、复制和粘贴的位置；扩展 API 无法移动这些内置项目，避免通过重复命令造成菜单更长。
